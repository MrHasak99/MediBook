...
async function seed() {
  ...
  try {
    ...
    if (!specialties || specialties.length === 0) {
      console.log('❌ No specialties found in database. Please run the SQL setup first.');
      return;
    }
    ...
    for (const docData of DOCTORS_DATA) {
      ...
      const user = existingUser?.users.find(u => u.email === docData.email);
      ...
      if (user) {
        ...
        // Give the trigger a moment to create the profile record
      ...
      // 3. Check if doctor record exists
